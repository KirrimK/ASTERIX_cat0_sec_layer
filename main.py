...
# creates a message made of the original message, the signature and the sha1 hash of the secret key concatenated
# in: message (bytes): the message to sign
# in: secret (bytes): the secret key to use
# out: message_with_signature_and_key_hash (bytes): the message with the signature and the secret key hash
...
# separates the message, the signature and the secret key from a message and verifies the signature
# in: big_message (bytes): the message to disassemble and verify
# out: message (bytes): the message
# out: is_verified (bool): True if the signature is valid and the integrity of the message is ok, False otherwise
